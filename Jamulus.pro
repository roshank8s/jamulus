...
contains(VERSION, .*dev.*)
{
    ...
    {
        VERSION = "$$VERSION" - nogit message ( "building version \"$$VERSION\" (intermediate without git repository)" )
    }
}
else { message ( "building version \"$$VERSION\" (release)" ) }

CONFIG += qt thread lrelease embed_translations debug_and_release
...
else { QT += widgets QT += multimedia }

#Do not set LRELEASE_DIR explicitly when using embed_translations.
#It doesn't work with multiple targets or architectures.
TRANSLATIONS =
    src / translation / translation_de_DE.ts src / translation / translation_fr_FR.ts src / translation / translation_ko_KR.ts src / translation /
    translation_pt_PT.ts src / translation / translation_pt_BR.ts src / translation / translation_es_ES.ts src / translation /
    translation_nb_NO.ts src / translation / translation_nl_NL.ts src / translation / translation_pl_PL.ts src / translation /
    translation_sk_SK.ts src / translation / translation_it_IT.ts src / translation / translation_sv_SE.ts src / translation /
    translation_zh_CN.ts

        INCLUDEPATH += src

        INCLUDEPATH_OPUS = libs / opus / include libs / opus / celt libs / opus / silk libs / opus / silk / float libs / opus / silk / fixed libs /
                           opus

#As JACK is used in multiple OS, we declare it globally
                               HEADERS_JACK = src / sound / jack / sound.h SOURCES_JACK = src / sound / jack /
                                                                                          sound.cpp
    ...
    win32
{
    DEFINES -= UNICODE #fixes issue with ASIO SDK ( asiolist.cpp is not unicode compatible ) DEFINES +=
        NOMINMAX #solves a compiler error in  qdatetime.h ( Qt5 ) RC_FILE = src / res / win - mainicon.rc mingw*
    {
        DEFINES += _WIN32_WINNT = 0x0600 #solves missing inet_pton in CSocket::SendPacket LIBS += -lole32 - luser32 - ladvapi32 - lwinmm - lws2_32
    }
    else
    {
        QMAKE_LFLAGS += / DYNAMICBASE : NO #fixes crash with libjack64.dll, see https
            : // github.com/jamulussoftware/jamulus/issues/93
              LIBS += ole32.lib user32.lib advapi32.lib winmm.lib ws2_32.lib greaterThan ( QT_MAJOR_VERSION, 5 )
        {
...
#main() entrypoint directly:
    ...
    contains ( CONFIG, "serveronly" ) { message ( Restricting build to server - only due to CONFIG += serveronly.) DEFINES += SERVER_ONLY }
    else
    {
        contains ( CONFIG, "jackonwindows" ){
            message ( Using JACK.) contains ( QT_ARCH, "i386" ){
                exists ( "C:/Program Files (x86)" ){ message ( "Cross compilation build" ) programfilesdir = "C:/Program Files (x86)" } else {
                    message ( "Native i386 build" ) programfilesdir = "C:/Program Files" } libjackname =
                    "libjack.lib" } else { message ( "Native x86_64 build" ) programfilesdir = "C:/Program Files" libjackname =
                                               "libjack64.lib" } !exists ( "$${programfilesdir}/JACK2/include/jack/jack.h" ){
                error ( "Error: jack.h was not found in the expected location ($${programfilesdir}). Ensure that the right JACK2 variant is "
                        "installed (32 Bit vs. 64 Bit)." ) }

            HEADERS += $$HEADERS_JACK SOURCES += $$SOURCES_JACK DEFINES += WITH_JACK DEFINES += JACK_ON_WINDOWS DEFINES +=
            _STDINT_H #supposed to solve compilation error in systemdeps.h INCLUDEPATH += "$${programfilesdir}/JACK2/include" LIBS +=
            "$${programfilesdir}/JACK2/lib/$${libjackname}" } else
        {
            message ( Using native Windows MIDI.)

                HEADERS += src / sound / midi - win / midi.h SOURCES +=
                src / sound / midi -
                win / midi.cpp

                      message ( Using ASIO.) message ( Please review the ASIO SDK licence.)

                          !exists ( libs / ASIOSDK2 / common ){ error (
                              "Error: ASIOSDK2 must be placed in Jamulus \\libs folder such that e.g. \\libs\ASIOSDK2\common exists." ) }
#Important : Keep those ASIO includes local to this build target in
#order to avoid poisoning other builds                    license - wise.
                      HEADERS += src / sound / asio / sound.h SOURCES +=
                src / sound / asio / sound.cpp libs / ASIOSDK2 / common / asio.cpp libs / ASIOSDK2 / host / asiodrivers.cpp libs / ASIOSDK2 / host /
                pc / asiolist.cpp INCLUDEPATH += libs / ASIOSDK2 / common libs / ASIOSDK2 / host libs / ASIOSDK2 / host / pc
        }
    }
}
else : macx
{
    contains ( CONFIG, "server_bundle" )
    {
        message ( The generated application bundle will run a server instance.)
            ...
            DEFINES += SERVER_BUNDLE TARGET = $${ TARGET } Server MACOSX_BUNDLE_ICON.files = src / res / mac - jamulus - server.icns RC_FILE =
                                                                                                 src / res / mac - jamulus - server.icns
    }
    else { MACOSX_BUNDLE_ICON.files = src / res / mac - mainicon.icns RC_FILE = src / res / mac - mainicon.icns }

    HEADERS += src / mac / activity.h src / mac / badgelabel.h OBJECTIVE_SOURCES += src / mac / activity.mm src / mac / badgelabel.mm CONFIG +=
        x86 QMAKE_TARGET_BUNDLE_PREFIX = app.jamulussoftware
...
#As of 2023 - 04 - 15 the macOS build with Xcode only fails.This is tracked in #1841
            ...
            contains ( CONFIG, "jackonmac" )
    {
        ...
        HEADERS += $$HEADERS_JACK SOURCES += $$SOURCES_JACK DEFINES += WITH_JACK DEFINES += JACK_REPLACES_COREAUDIO INCLUDEPATH +=
            / usr / local / include LIBS += / usr / local / lib / libjack.dylib
    }
    else
    {
        message ( Using CoreAudio.) HEADERS += src / sound / coreaudio - mac / sound.h SOURCES += src / sound / coreaudio - mac / sound.cpp
    }
}
else : ios
{
    QMAKE_ASSET_CATALOGS += src / res / iOSIcons.xcassets QMAKE_INFO_PLIST = ios / Info.plist OBJECTIVE_SOURCES +=
        src / ios / ios_app_delegate.mm HEADERS += src / ios / ios_app_delegate.h HEADERS +=
        src / sound / coreaudio - ios / sound.h OBJECTIVE_SOURCES += src / sound / coreaudio - ios / sound.mm QMAKE_TARGET_BUNDLE_PREFIX =
                                                                         app.jamulussoftware LIBS += -framework AVFoundation - framework AudioToolbox
}
else : android
{
    ANDROID_ABIS = armeabi - v7a arm64 - v8a x86 x86_64 ANDROID_VERSION_NAME = $$VERSION ANDROID_VERSION_CODE =
                       $$system ( git log-- oneline | wc - l )
...
#liboboe requires C++ 17 for std::timed_mutex
        ...
        QT += androidextras
            ...
            DEFINES += ANDROIDDEBUG
                           ...
                           target.path = / tmp / your_executable #path on device INSTALLS += target

                                             HEADERS += src / sound / oboe /
                                                        sound.h

                                                            SOURCES += src / sound / oboe / sound.cpp src / android /
                                                                       androiddebug.cpp

                                                                           LIBS += -lOpenSLES ANDROID_PACKAGE_SOURCE_DIR =
                                                                           $$PWD / android    DISTFILES +=
...
#    make sure you git pull with submodules to pull the latest Oboe library
                      OBOE_SOURCES = $$files ( libs / oboe / src /*.cpp, true)
                         ...
                         CLANG_FORMAT_SOURCES ~= s!^\(libs/.*/
                                               | src / res / qrc_resources\.cpp\)\S* $ !!g clang_format.commands =
            'clang-format -i $$CLANG_FORMAT_SOURCES' QMAKE_EXTRA_TARGETS += clang_format
